(function () {
    const PODNAPISI_URL = httpswww.podnapisi.netsubtitlessearch;
    ...
    function parseSearchResults(html) {
        ...
        const doc = parser.parseFromString(html, texthtml);
        const results = [];
        doc.querySelectorAll(.subtitle-entry).forEach(entry = {
            const title = entry.querySelector(.title).textContent.trim();
            const downloadUrl = entry.querySelector(a[href='endownloadsubtitle']).href;
    ...
    function attachSubtitles(subtitleContent) {
        const subtitleBlob = new Blob([subtitleContent], { type textsrt });
